def set_state_profile(prefix, df, unique_fields, state_profile, escheat_date_field_name, escheat_due_date_field_name, state_profiles_prefix):
    ...
    state_profile_df = (
        ...
        .withColumn(
            ...
            when(
                ...
                .over(state_partition.rowsBetween(1, Window.unboundedFollowing))
                .isNotNull(),
                0
            )
    ...
    state_profile_df = (
        ...
        .select(
            ...
            .when(
                coalesce(
                    last(col(escheat_date_field_name), ignorenulls=True).over(state_partition.rowsBetween(Window.unboundedPreceding, 0)),
                    col('eDateToUse')
                ).isNull(),
                lit(None)
            )
            ...
            .when(
                col(f'{state_profiles_prefix}EffectiveDate') <= coalesce(
                    last(col(escheat_date_field_name), ignorenulls=True).over(state_partition.rowsBetween(Window.unboundedPreceding, -1)),
    ...
    state_profile_df = (
        ...
        .withColumn(
            ...
            when(
                ...
                .over(state_partition.rowsBetween(Window.unboundedPreceding, -1)),
                0
            )
